...
gene_poe_ridge = function(ase_data, gene_symbols, data_type) {
  ...
  ase_data = subset(
    ase_data,
    (genes_exonic_symbol %in% gene_symbols) & (!is.na(PatFreq))
  )
  
  if (length(unique(as.character(ase_data$genes_exonic))) == 0) {
    stop("None of the genes is found")
  }
  
  counts_bygene = aggregate(
    subset(ase_data, select = c(PatDepth, MatDepth, totalCount)),
    by = list(ase_data$RNAid, ase_data$genes_exonic_symbol),
    sum
  )
  ...
  poe_ridge = ggplot(counts_bygene, aes(x = data_to_plot, y = gene_exonic, fill = after_stat(x))) +
    ...
    scale_fill_gradientn(
      name = x_lab,
      limits = c(0, 1),
      colours = c("deeppink", "white", "deepskyblue")
    ) +
    theme(
