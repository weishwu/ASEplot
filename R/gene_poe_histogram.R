...
gene_poe_histogram = function(ase_data, gene_symbol, data_type, binwidth = NULL, sample_name = NULL) {
  ...
  snp_data = subset(
    ase_data,
    (genes_exonic_symbol == gene_symbol) & (!is.na(PatFreq))
  )
  ...
  counts_bygene = aggregate(
    subset(snp_data, select = c(PatDepth, MatDepth, totalCount)),
    by = list(snp_data$RNAid, snp_data$genes_exonic),
    sum
  )
  colnames(counts_bygene)[1:2] = c('RNAid', 'gene_exonic')
  counts_bygene$PatFreq = counts_bygene$PatDepth / counts_bygene$totalCount
  ...
  if (!is.null(sample_name)) {
    poe_histogram = ggplot(counts_bygene, aes(x = data_to_plot)) +
      ...
      theme_bw() +
      geom_vline(
        data = counts_bygene[counts_bygene$RNAid == sample_name, ],
        aes(xintercept = data_to_plot, col = RNAid),
        linewidth = 1,
        linetype = 'dashed'
      ) + 
      xlab(x_lab) + 
