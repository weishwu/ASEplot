...
snp_gene_ase_boxplot = function(ase_data, gene_symbol, data_type) {
  ...
  snp_data = snp_data %>%
    inner_join(agg_, by = 'RNAid') %>%
    mutate(SNP_position = paste(contig, position, strand, sep = '_')) %>%
    mutate(RNAid_2 = as.character(RNAid))
  ...
  ase_boxplot = ggplot(snp_data, aes(x = reorder(RNAid_2, data_to_plot, FUN = median), y = data_to_plot)) +
    ...
    theme_classic() +
    theme(
      axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = x_axis_text_size),
