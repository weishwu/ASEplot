...
ase_data_in_gene = function(ase_data, exons_all, gene_symbol) {
  ...
  gene_data_allsamples = subset(ase_data, genes_exonic_symbol == gene_symbol)
  ...
  exons = as.data.frame(
    exons_all[exons_all[, 6] == gene_symbol, , drop = FALSE],
    stringsAsFactors = FALSE
  )
  exons[, 2:3] = lapply(exons[, 2:3], function(x) {
  ...
  for (i in 1:nrow(exons_nospace)) {
    ...
    approxfun_list[[i]] = approxfun(
      range(c(exons_nospace[i, 1], exons_nospace[i, 2])),
      c(exons_nospace[i, 3], exons_nospace[i, 4])
    )
  }
  
  positions = unique(c(
    as.numeric(gene_data_allsamples$position),
    as.numeric(exons[, 2]),
    as.numeric(exons[, 3])
  ))
  ...
  interpolate_fun = function(p) {
    as.numeric(position_map[position_map[, 1] == p, 2])
  }
  
  gene_data_allsamples$interpolated = unlist(
    lapply(as.numeric(gene_data_allsamples$position), interpolate_fun)
  )
