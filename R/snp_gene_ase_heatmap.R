...
#' @return A ggplot in which each column is a SNP site and each row is a sample.
#' @examples
#' data('ase_data.test')
#' ase_df = ase_data$ase_df
#' exons = ase_data$union_exons_per_gene
#' snp_gene_ase_heatmap(ase_df, exons, "RHOBTB3", 'pat-freq')
#' @export
snp_gene_ase_heatmap = function(ase_data, exons_all, gene_symbol, data_type) {
  ...
  gene_type = unique(
    matrix(
      unlist(strsplit(as.character(snp_data$exons_merged), ';')),
      nrow(snp_data), 7, byrow = TRUE
    )[,7]
  )
  ...
  exon_blocks = data.frame(
    x1 = c(exons_nospace[,3]),
    x2 = c(exons_nospace[,4]),
    y1 = -0.01,
    y2 = 1.01,
    t = rep(c('a','b'), 1e6)[1:nrow(exons_nospace)]
  )
  ...
  snp_matrix = with(
    snp_data,
    tapply(data_to_plot, list(as.character(RNAid), interpolated), sum)
  )
  
  htdata = expand.grid(
    X = as.character(colnames(snp_matrix)),
    Y = as.character(rownames(snp_matrix))
  )
  ...
  htdata = merge(
    htdata,
    aggregate(rep(1, nrow(snp_data)), by = list(snp_data$RNAid), sum),
    by.x = 2, by.y = 1
  )
  ...
  ase_heatmap = ggplot(htdata, aes(X, Y)) + 
    ...
    labs(
      title = paste(
        "sample count: ", length(unique(as.character(snp_data$RNAid))),
        "; SNP count: ", length(unique(as.numeric(snp_data$position))),
        sep = ""
      ),
      y = "RNA samples",
      x = "SNPs",
