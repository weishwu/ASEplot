...
snp_gene_ase_scatter = function(ase_data, exons_all, gene_symbol, data_type, sample_name = NULL) {
  ...
  if (!is.null(sample_name)) {
    ...
    snp_data = snp_data %>%
      mutate(samples = case_when(RNAid == sample_name ~ sample_name, TRUE ~ "others"))
    
    snp_data$samples = factor(snp_data$samples, levels = c('others', sample_name))
    snpCount = sum(snp_data$samples == sample_name)
    median_val = format(
      round(median(snp_data$data_to_plot[snp_data$samples == sample_name], na.rm = TRUE), 2),
      nsmall = 2
    )
    
    scatterplot = ggplot(data = snp_data %>% filter(samples == 'others'), 
                         aes(x = interpolated, y = data_to_plot, col = samples, shape = samples, alpha = samples)) + 
      geom_point(size = 5) + 
      geom_point(data = snp_data %>% filter(samples != 'others'), 
                 aes(x = interpolated, y = data_to_plot, col = samples, shape = samples, alpha = samples), 
      ...
      labs(
        title = paste(
          ...
          "; Gene type: ", gene_type,
          "; strand: ", gene_strand,
          "\nRNA sample: ", sample_name,
          "; SNP count: ", snpCount,
          "; median: ", median_val,
          sep = ""
        ),
        y = y_lab,
        x = 'exon boundaries (introns not shown)'
      ) +
